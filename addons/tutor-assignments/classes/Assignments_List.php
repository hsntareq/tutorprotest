...
class Assignments_List extends \Tutor_List_Table {
	...
	function column_duration($item) {
		...
		echo $value ? ($value . ' ' . __( $time, 'tutor-pro' ) . ($value>1 ? 's' : '')) : __('No Limit', 'tutor-pro');
	}
	...
	function column_action($item){
		...
		echo '<a data-assignment_id="'. $item->comment_ID.'" data-assignment_action="delete" data-warning_message="'. __('Sure to delete the assignment? Attached files also will be deleted.', 'tutor-pro').'" href="#" class="tutor-assignment-action button button-delete" data-toast_success_message="'.__('Deleted', 'tutor-pro').'">'._x(' Delete', 'assignment delete', 'tutor-pro').'</a>';
	}
	...
	function prepare_items() {
		...
		$date !== '' ? $date = "  AND DATE(comment.comment_date) = CAST( '$date' AS DATE ) " : '';
