...
class ExportAnalytics {
    ...
    protected function students_data() {
        ...
        $students = $wpdb->get_results( $wpdb->prepare(
            " SELECT DATE_FORMAT(users.user_registered, '%%d %%b %%Y %%T') AS register_date, users.ID, users.display_name, course.post_title AS enrolled_course, course.ID as course_id
        ...
        foreach( $students as $student ) {
            ...
            $total_assignments    = tutor_utils()->get_assignments_by_course( $course_id )->count;
            $completed_assignment = tutor_utils()->get_completed_assignment( $course_id, $student_id );
            $total_quiz           = Analytics::get_all_quiz_by_course( $course_id );
