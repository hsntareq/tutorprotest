...
if($_date){
    $date_filter = DateTime::createFromFormat('Y-m-d', $_date != '' ? tutor_get_formated_date('Y-m-d', $_date) : '');
...
<div class="tutor-admin-search-box-container">
        ...
        <div>
            <select class="tutor-report-category">
                <?php
                    ...
                    if (!empty($terms)) {
                        array_unshift($terms, (object)['term_id' => '', 'name' => __( 'All', 'tutor-pro' )]);
    ...
    <div>
        ...
        <div>
            <select class="tutor-report-sort">
                <option <?php selected( $order_filter, 'ASC' ); ?>>
                    <?php _e( 'ASC', 'tutor-pro' );?>
                </option>
                <option <?php selected( $order_filter, 'DESC' ); ?>>
                    <?php _e( 'DESC', 'tutor-pro'); ?>
                </option>
            </select>
        </div>
    </div>
    ...
    <div>
        ...
        <div class="date-range-input">
            <input type="text" class="tutor_date_picker tutor-report-date" value="<?php echo '' !== $_date ? tutor_get_formated_date( get_option( 'date_format' ), $_date ) : ''; ?>" autocomplete="off" placeholder="<?php _e( get_option( 'date_format' ), 'tutor-pro' ); ?>" />
...
<div class="tutor-list-wrap tutor-report-course-list">
    ...
    <div class="tutor-list-footer">
        <div class="tutor-report-count">
            <?php 
                if($total_items > 0){
                    printf( __('Items <strong> %s </strong> of <strong> %s </strong> total', 'tutor-pro' ), $per_page,  $total_items );
                }
            ?>
        </div>
