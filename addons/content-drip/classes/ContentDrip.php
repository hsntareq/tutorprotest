...
class ContentDrip {
	...
	public function __construct() {
		...
		*/
		...
		add_filter( 'tutor_pro/email/list', array( $this, 'register_emails' ), 11 );
	...
	public function is_lock_lesson( $post = null ) {
		...
		if ( $drip_type === 'unlock_by_date' ) {
			...
			if ( $unlock_timestamp ) {
				...
				$this->unlock_message = sprintf( __( 'This %1$s will be available from %2$s', 'tutor-pro' ), $singular_post_type, $unlock_date );
	...
	public function register_emails( $attr ) {
		...
		$attr['email_to_students']['new_lesson_published'] = array(
			...
			'course_title' => __( 'New course title', 'tutor-pro' ),
			'message'      => __( 'We are glat to inform you that new lesson added on course {course_title}.', 'tutor-pro' ),
			'footer-text'  => __( 'Reply to this email to communicate with the instructor.', 'tutor-pro' ),
		);

		$attr['email_to_students']['new_quiz_published'] = array(
		...
		$attr['email_to_students']['new_assignment_published'] = array(
			'label'        => __( 'New Quiz Published', 'tutor-pro' ),
