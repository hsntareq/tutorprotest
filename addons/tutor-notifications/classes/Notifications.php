...
class Notifications {
    ...
    public function __construct() {
		...
		add_action( 'tutor_enrollment_cancelled', array( $this, 'tutor_student_remove_from_course' ), 10, 3 );
    }
    ...
    public function instructor_approval( $instructor_id ) {
        ...
        $message_content = sprintf( _x( '<span class="color-text-title">Congratulations</span> %s, <span class="color-text-title">your application to be an instructor has been approved.</span>', 'instructorship-approved-text', 'tutor-pro' ), ucfirst( $display_name ) );
    ...
    public function instructor_rejected( $instructor_id ) {
        ...
        $message_status  = 'UNREAD';
        $message_title   = __( 'Instructorship', 'tutor-pro' );
        $message_content = sprintf( _x( '%s, <span class="color-text-title">your instructorship application has been declined.</span>', 'instructorship-rejected-text', 'tutor-pro' ), ucfirst( $display_name ) );
    ...
    public function new_instructor_application( $instructor_id ) {
        ...
        $admin_id        = get_user_by( 'email', get_option( 'admin_email' ) )->ID;
        ...
        $message_status  = 'UNREAD';
        $message_title   = __( 'Instructorship', 'tutor-pro' );
        $message_content = sprintf( _x( '%s, <span class="color-text-title">you have received a new application from</span> %s <span class="color-text-title">for Instructorship.</span>', 'instructor-application-received', 'tutor-pro' ), ucfirst( $admin_name ), ucfirst( $display_name ) );
    ...
    public function tutor_after_assignment_evaluated( $assignment_submission_id ) {
        ...
        $submitted_assignment = tutils()->get_assignment_submit_info( $assignment_submission_id );
        ...
        $message_content      = sprintf( _x( '<span class="color-text-title">Hi</span> %s, <span class="color-text-title">your</span> %s <span class="color-text-title">has been graded. Check it out.</span>', 'grades-submitted-text', 'tutor-pro' ), ucfirst( $display_name ), $assignment_name );
    ...
    public function tutor_announcements_notify_students( $announcement_id, $announcement, $action_type ) {
		...
		if ( ! isset( $_POST['tutor_notify_all_students'] ) || ! $_POST['tutor_notify_all_students'] || ! $notification_enabled ) return;
        ...
        $message_content  = sprintf( _x( '<span class="color-text-title">%s</span> %s <span class="color-text-title">of</span> %s.', 'announcement-text', 'tutor-pro' ), $ann_title, ucfirst( $author_name ), $course_name );
    ...
    public function tutor_after_answer_to_question( $answer_id ) {
        ...
        $message_content = sprintf( _x( '<span class="color-text-title">A new answer has been posted by</span> %s <span class="color-text-title">in</span> %s\'s Q&A.', 'qa-answer-posted', 'tutor-pro' ), ucfirst( $comment_author ), $course_name );
    ...
    public function feedback_submitted_for_quiz_attempt( $attempt_id ) {
        ...
        $message_content = sprintf( _x( '<span class="color-text-title">Your quiz result for</span> %s <span class="color-text-title">of</span> %s <span class="color-text-title">has been published.</span>', 'quiz-attempt-text', 'tutor-pro' ), $quiz_title, $course->post_title );
    ...
    public function tutor_student_course_enrolled( $course_id, $user_id, $enrollment_id ) {
        ...
        $display_name    = $user_data->display_name;
        $course          = tutils()->get_course_by_enrol_id( $enrollment_id );
        ...
        $message_content = sprintf( _x( '<span class="color-text-title">Congratulations, you have been successfully enrolled in</span> %s.', 'got-enrolled-text', 'tutor-pro' ), $course_title );
     ...
     */
    public function tutor_student_remove_from_course( $course_id, $user_id, $enrollment_id ) {
        global $wpdb;
        ...
        $message_status  = 'UNREAD';
        $message_title   = __( 'Enrollments', 'tutor-pro' );
        $message_content = sprintf( _x( '%s, <span class="color-text-title">your enrollment request for</span> %s <span class="color-text-title">has been declined.</span>', 'enrollment-cancelled-text', 'tutor-pro' ), ucfirst( $display_name ), $course_title );
