...
class Pusher extends Push_Notification {
    ...
    public function new_instructor_application( $instructor_id ) {
        ...
        $this->broadcast( $admin_id, __( 'New Instructor Request', 'tutor-pro' ), $message, $instructor_page_url );
    }
    ...
    public function tutor_announcements_notify_students( $announcement_id, $announcement, $action_type ) {
        ...
        $student_ids = tutils()->get_students_data_by_course_id( $announcement->post_parent, 'ID' );
    ...
    public function tutor_after_answer_to_question( $answer_id ) {
		...
		$qa_url = get_the_permalink( $answer->comment_post_ID ) . 'questions/';
    ...
    public function feedback_submitted_for_quiz_attempt( $attempt_id ) {
        ...
        $this->broadcast( $attempt->user_id, $title, $instructor_feedback, tutils()->get_tutor_dashboard_page_permalink( 'my-quiz-attempts' ) );
    }
    ...
    public function course_enroll_email_to_student( $course_id, $student_id, $enrol_id, $status_to='completed' ) {
        ...
        $course_url = tutils()->get_course_first_lesson( $course_id );
    ...
    public function tutor_student_remove_from_course( $enrol_id ) {
        ...
        $title = $enrolment->status == 'cancel' ? __( 'Enrolment has been cancelled.', 'tutor-pro' ) : __( 'You\'ve Been Removed From a Course.', 'tutor-pro' );
