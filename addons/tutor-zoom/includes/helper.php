 ...
 * @return array
 */
function get_tutor_zoom_meetings($args=array()) {
    $query = array(
        'posts_per_page'    => -1,
        'post_type'         => 'tutor_zoom_meeting',
        'meta_key'          => '_tutor_zm_start_datetime',
        'orderby'           => 'meta_value datetime',
        'order'             => 'asc'
    );
    ...
    if (isset($args['course_id']) && !empty($args['course_id'])) {
        $query['post_parent'] = $args['course_id'];
    }
    if (isset($args['orderby'])) {

        $query['orderby'] = $args['orderby'];

        if( $query['orderby'] == 'datetime' ) {
            $query['meta_key'] = '_tutor_zm_start_datetime';
            $query['orderby'] = 'meta_value datetime';
        }
    }
    if (isset($args['order'])) {
        $query['order'] = strtolower($args['order']) == 'asc' ? 'asc' : 'desc';
    }

    if (isset($args['date']) && !empty($args['date'])) {
        $args['date'] = $args['date'] != '' ? tutor_get_formated_date( 'Y-m-d', $args['date'] ) : '';
        $query['meta_query'] = array(
            array(
                'key'     => '_tutor_zm_start_date',
                'value'   => $args['date'],
                'compare' => '=',
                'type'    => 'DATE'
            )
    ...
    if ( get_option('tutor_zoom_sync') ) {
        if ( isset($args['filter']) && $args['filter'] != '' ) {
            date_default_timezone_set('UTC');
            $compare = $args['filter'] == 'active' ? '>' : '<';
            $query['meta_query'] = array(
                array(
                    'key'     => '_tutor_zm_start_datetime_with_duration',
                    'type'    => 'DATETIME',
    ...
    $meetings           = get_posts($query);

    $meetings_with_info = [];
    //add expired & start info
    foreach ( $meetings as $meeting ) {
        $is_started             = tutor_zoom_meeting_data( $meeting->ID )->is_started;
        $is_expired             = isset($args['filter']) && $args['filter'] === 'expired' ? true : false;
        $meeting->is_expired    = $is_expired;
        $meeting->is_started    = $is_started;
        array_push( $meetings_with_info, $meeting ); 
    }

    return $meetings_with_info;    
}
 ...
 * @return array
 */
function get_tutor_zoom_webinars() {
    $args = array(
        'numberposts' => -1,
        'post_type' => 'tutor_zoom_webinar'
    );
 ...
 * @return bool
 *
 * Check if the current page is single zoom meeting page
 *
 * @since v.1.8.1
 */
if ( ! function_exists('is_single_tutor_zoom_meeting_page')) {
    function is_single_tutor_zoom_meeting_page() {
        global $wp_query;
        if (is_single() && !empty($wp_query->query['post_type']) && $wp_query->query['post_type'] === 'tutor_zoom_meeting') {
            return true;
        }
        return false;
    }
