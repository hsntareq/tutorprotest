...
/*!*********************************************************************!*\
  !*** ./addons/google-classroom/assets/react/classroom-dashboard.js ***!
  \*********************************************************************/
...
window.jQuery(document).ready(function ($) {
  ...
  var credential = function credential() {
    ...
    this.on_drop = function () {
      var _this = this; // Process drag and drop file
    ...
    this.on_save = function () {
      ...
      this.load_button.click(function () {
        ...
        form.append('action', 'tutor_gc_credential_save'); // Append nonce to the object manually, ajaxSetup doen't as it is formadata

        var nonce = tutor_get_nonce_data(true);
        form.append(nonce.key, nonce.value);
  ...
  var toggle_bulk_action_state = function toggle_bulk_action_state(state_enable) {
  ...
  var pop_up_confirmation = function pop_up_confirmation(icon, title, description, buttons, callback) {
    var container = $('<div class="tutor-gc-pop-up-container">\
                    <div>\
                        <img src="' + window._tutorobject.tutor_pro_url + 'addons/google-classroom/assets/images/' + icon + '"/>\
                        <h3>' + title + '</h3>\
                        <p>' + description + '</p>\
  ...
  var import_confirmation = function import_confirmation(callback) {
    ...
    var buttons = [{
      "class": 'tutor-gc-button-secondary',
      text: 'No',
      action: 'no'
    }, {
      "class": 'tutor-gc-button-primary',
      text: 'Yes, Import Student',
      action: 'yes'
    }];
  ...
  var delete_confirmation = function delete_confirmation(callback) {
  ...
  classlist.find('[data-action]').not('a').click(function (e) {
    ...
    var error_message = function error_message(e) {
    ...
    function internal() {
      ...
      $.ajax({
        ...
        success: function success(r) {
          try {
            r = JSON.parse(r);
          } catch (e) {} // Show error message if invalid response
  ...
  $('#tutor_gc_classroom_code_privilege').change(function () {
    ...
    $.ajax({
      url: window._tutorobject.ajaxurl,
      data: {
        action: 'tutor_gc_classroom_code_privilege',
        enabled: value
      },
      type: 'POST',
      success: function success() {},
  ...
  $('#tutor_gc_credential_upgrade').click(function (e) {
    ...
    $.ajax({
      url: window._tutorobject.ajaxurl,
      data: {
        action: 'tutor_gc_credential_upgrade'
      },
      type: 'POST',
      success: reload,
      error: reload
...
/******/ })()
;
//# sourceMappingURL=classroom-dashboard.js.map