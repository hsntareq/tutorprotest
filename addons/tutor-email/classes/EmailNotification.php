...
class EmailNotification {
	 ...
	 * @return JSON
	 */
	public function save_email_templateY() {
		tutils()->checking_nonce();

		$to                           = isset( $_POST['to'] ) ? sanitize_text_field( $_POST['to'] ) : null;
		$key                          = isset( $_POST['key'] ) ? sanitize_text_field( $_POST['key'] ) : null;
		$email_data['get_email_data'] = ( new EmailData() )->get_recipients();
		// $email_request                   = $email_data['get_email_data'][ $to ][ $key ];
		$email_request = $email_data['get_email_data'];

		$tutor_email_options = array();
		$tutor_email_options = get_option( 'email_template_data' );
		$message             = $_POST['email-additional-message'];
		$email_option        = ! empty( $tutor_email_options ) ? $tutor_email_options : array();
		// $pattern             = '/<\/?div[^>]*\>/g';
		// $message             = preg_replace( $pattern, '', $message );
		$recipient_data = $email_request[ $to ][ $key ];

		/*
		 $email_key['logo']          = sanitize_textarea_field( $_POST['email-title-logo'] );
		$email_key['subject']       = sanitize_text_field( $_POST['email-subject'] );
		$email_key['heading']       = sanitize_text_field( $_POST['email-heading'] );
		$email_key['message']       = sanitize_textarea_field( htmlentities( $message ) );
		$email_key['before-button'] = sanitize_textarea_field( $_POST['email-before-button'] );
		$email_key['footer-text']   = sanitize_textarea_field( $_POST['email-footer-text'] ); */

		$email_option_data = $email_option[ $to ][ $key ];
		foreach ( $recipient_data as $key => $recipient ) {
			$email_key[ $key ] = $email_option_data[ $key ];
		}
		// unset( $email_key['before-button'] );
		pr( $_REQUEST );
		die;
		if ( ! empty( $email_option_data ) ) {

			foreach ( $email_option_data as $key => $email_data ) {
				if ( isset( $email_key[ $key ] ) ) {
					$email_output[ $key ] = array_merge( $email_option_data[ $key ], $email_key[ $key ] );
				} else {
					$email_output[ $key ] = $email_option_data[ $key ];
				}
			}

			$tutor_email_options = ( ! array_key_exists( $to, $email_output ) ) ? array_merge( $email_output, $email_key ) : $email_output;

		} else {
			$tutor_email_options = array_merge( $email_option_data, $email_key );
		}
		// $tutor_email_options = array();

		update_option( 'email_template_data', $tutor_email_options );
		wp_send_json_success( $tutor_email_options );
	...
	public function save_email_template() {
		...
		$email_request[ $to ][ $key ] = array(
			'logo'          => sanitize_textarea_field( $_POST['email-title-logo'] ),
			'subject'       => sanitize_text_field( $_POST['email-subject'] ),
			'heading'       => sanitize_text_field( $_POST['email-heading'] ),
			'message'       => sanitize_textarea_field( htmlentities( $message ) ),
			'footer-text'   => sanitize_textarea_field( $_POST['email-footer-text'] ),
			'block-heading' => sanitize_textarea_field( $_POST['email-block-heading'] ),
			'block-content' => sanitize_textarea_field( $_POST['email-block-content'] ),
			'before-button' => sanitize_textarea_field( $_POST['email-before-button'] ),
		);
	...
	public function send_test_email_ajax() {
		...
		$replacable     = array(
			'{email_heading}'       => $recipient_data['heading'],
			'{email_message}'       => $recipient_data['message'],
			'{student_name}'        => $current_user->display_name,
			'{student_username}'    => $current_user->display_name,
			'{student_email}'       => $current_user->user_email,
			'{site_url}'            => get_bloginfo( 'url' ),
			'{site_name}'           => get_bloginfo( 'name' ),
			'{course_name}'         => $recipient_data['course_name'],
			'{student_url}'         => get_author_posts_url( $user_id ),
			'{course_title}'        => $recipient_data['course_title'],
			'{footer_text}'         => $recipient_data['footer-text'],
			'{question}'            => $recipient_data['question'],
			'{enroll_time}'         => $recipient_data['enroll_time'],
			'{instructor_username}' => $recipient_data['instructor_username'],
			'{logo}'                => TUTOR_EMAIL()->url . 'assets/images/logo.svg',
			'{answer_by}'           => $recipient_data['answer_by'],
			'{answer_date}'         => $recipient_data['answer_date'],
			'{before_button}'       => $recipient_data['before-button'],
			'{username}'            => $recipient_data['username'],
			'{quiz_name}'           => $recipient_data['quiz_name'],
			'{instructor_email}'    => $recipient_data['instructor_email'],
			'{instructor_name}'     => $recipient_data['instructor_name'],
			'{block_heading}'       => $recipient_data['block-heading'],
			'{block_content}'       => $recipient_data['block-content'],
			'{assignment_title}'    => $recipient_data['assignment_title'],
			'{withdraw_amount}'     => $recipient_data['withdraw_amount'],
		);
		$replace_keys   = array_keys( $replacable );
		$replace_values = array_values( $replacable );

		$subject = sprintf( __( $recipient_data['subject'] . ' %s', 'tutor-pro' ), get_bloginfo( 'name' ) );

		ob_start();
		$this->tutor_load_email_template( $email_template );

		$email_tpl = apply_filters( 'tutor_email_tpl/course_completed', ob_get_clean() );
		$email_tpl = apply_filters( 'tutor_certificate_add_url_to_email', $email_tpl, 1 );

		$message = $this->get_message( $email_tpl, $replace_keys, $replace_values );

		$header = 'Content-Type: ' . $this->get_content_type() . "\r\n";
		$header = apply_filters( 'student_course_completed_email_header', $header, 1 );

		echo $message;

		$this->send( $testing_email, $subject, $message, $header );
		// $this->send( $current_user->user_email, $subject, $message, $header );
	}
	...
	public function course_complete_email_to_student( $course_id ) {
		...
		$this->send( $student->user_email, $this->get_subject( 'to_student_course_completed', $subject ), $message, $header );
	}
	...
	public function course_complete_email_to_teacher( $course_id ) {
		...
		$subject = sprintf( __( '%1$s just completed the course %2$s', 'tutor-pro' ), $student->display_name, $course->post_title );
	...
	public function quiz_finished_send_email_to_student( $attempt_id ) {
		...
		$subject = apply_filters( 'student_quiz_completed_email_subject', sprintf( __( 'Thank you for %s answers, we have received', 'tutor-pro' ), $quiz_name ) );
		$header  = 'Content-Type: ' . $this->get_content_type() . "\r\n";
	...
	public function course_enroll_email_to_teacher( $course_id, $student_id, $enrol_id, $status_to = 'completed' ) {
		...
		$subject = sprintf( __( '%1$s enrolled in the course %2$s', 'tutor-pro' ), $student->display_name, $course->post_title );
	...
	public function course_enroll_email_to_student( $course_id, $student_id, $enrol_id, $status_to = 'completed' ) {
		...
		$subject = sprintf( __( 'You are enrolled in the course %1$s at %2$s', 'tutor-pro' ), $course->post_title, $site_url );
	...
	public function tutor_after_add_question( $course_id, $comment_id ) {
		...
		$subject = sprintf( __( '%1$s asked a question on %2$s', 'tutor-pro' ), $student->display_name, $course->post_title );
	...
	public function tutor_lesson_completed_after( $lesson_id ) {
		...
		$subject = sprintf( __( '%1$s just completed the lesson %2$s', 'tutor-pro' ), $student->display_name, $course->post_title );
	...
	public function tutor_assignment_after_submitted( $assignment_submit_id ) {
		...
		$instructor_name      = get_the_author_meta( 'display_name', $submitted_assignment->post_author );
		...
		$subject = sprintf( __( 'New Assignment Submission on course %1$s at %2$s', 'tutor-pro' ), $course_name, $site_name );
	...
	public function tutor_after_assignment_evaluate( $assignment_submit_id ) {
		...
		$subject = sprintf( __( 'Grade submitted for Assignment %1$s on %2$s', 'tutor-pro' ), $assignment_name, $course_name );
	...
	public function feedback_submitted_for_quiz_attempt( $attempt_id ) {
		...
		$block_heading       = $feedback_submitted_for_quiz['block-heading'];
		$block_content       = $feedback_submitted_for_quiz['block-content'];
	...
	*/
	...
	public function new_lqa_published( $lqa ) {
		...
		$subject   = sprintf( __( 'New %s Published', 'tutor-pro' ), __( $lqa['lqa_type'], 'tutor-pro' ) );
	...
	public function tutor_course_update_notification( $post_id, $post, $update ) {
		...
		// $is_enable_reject_mail    = tutor_utils()->get_option( 'email_to_teachers.instructor_course_reject' );
		...
		if ( tutor_utils()->is_instructor( $course->post_author ) && 'publish' === $course_status ) {
			...
			if ( ! $post_meta ) {
				...
				if ( 'draft' === $course_status && $is_enable_publish_mail ) {
					...
					$subject           = sprintf( __( 'Course Content Review Request - %s', 'tutor-pro' ), $course_title );
					$file_tpl_variable = array(
						'{email_logo}',
						'{instructor_name}',
						'{site_name}',
					...
					$this->tutor_send_course_update_notification(
						$author_email,
						$template,
						$file_tpl_variable,
						$replace_data,
				...
				if ( 'publish' === $course_status && $is_enable_publish_mail ) {
					...
					$subject           = sprintf( __( 'Your course has been published - %s', 'tutor-pro' ), $course_title );
					$file_tpl_variable = array(
						'{email_logo}',
						'{instructor_name}',
						'{site_name}',
