        ...
        class Update {
            ...
            public function check_for_update_api() {
                ...
                if (!is_wp_error($request) || wp_remote_retrieve_response_code($request) === 200) {
                    ...
                    if(is_object($request_body) && property_exists($request_body, 'dependency_products') && property_exists($request_body, 'option_keymap')) {
                        $dependency_products = array();
                        $option_keymap = (array)$request_body->option_keymap;
                        
                        // Loop through all the dependencies and prepare license keys
                        foreach((array)$request_body->dependency_products as $dependency) {
                        ...
                        $params['dependency_products'] = $dependency_products;
                        $request = wp_remote_post($this->api_end_point . $access_slug, $params);
