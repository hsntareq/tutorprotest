	...
	<div style="background-image: url(<?php echo esc_url( tutor()->v2_img_dir ); ?>email-heading.svg);background-position: top right;background-repeat: no-repeat;padding: 50px;">
		...
		<p><?php _e( 'Dear {instructor_username},', 'tutor-pro' ); ?></p>
			...
			<div data-source="email-additional-message">
				<p>
					<?php _e( '<strong>{student_username}</strong> has recently completed the lesson <strong>{lesson_name}</strong> of <strong>{course_name}</strong> at <strong>{completion_time}</strong>. The completed lesson URL is: <strong>{lesson_url}</strong>.', 'tutor-pro' ); ?>
				</p>
			</div>
		</div>
